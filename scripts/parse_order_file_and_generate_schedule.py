            ...
            if 'break' in session_string.lower() or 'lunch' in session_string.lower():
                ...
                for split in day_session_splits:
                    ...
                    if 'poster' in session_type.lower():
                        ...
                        generated_html.append('<div class="session session-expandable session-papers{}" id="session-{}"><div id="expander"></div><a href="#" class="session-title">{}: {}</a><br/><span class="session-time">{} &ndash; {}</span><br/><span class="session-location btn btn--info btn--location">Gold Hall</span><br/><div class="paper-session-details"><br/><a href="#" class="session-selector" id="session-{}-selector"> Choose All</a><a href="#" class="session-deselector" id="session-{}-deselector">Remove All</a><table class="paper-table"><tr><td class="session-chair" colspan="2">Chair: TBD</td></tr>'.format(next(paper_session_counter), session_id.lower(), session_id, session_title, session_group_start, session_group_end, session_id.lower(), session_id.lower()))
                    for paper in split:
                        if 'poster' in session_type.lower():
